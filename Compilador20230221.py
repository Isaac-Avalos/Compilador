...
def erra(terr, desc):
    global ren, col
    global cERR
    print('['+str(ren)+']'+'['+str(col)+']', terr, desc)
    cERR = True
...
def blkFunc():
    global lex, tok
    if lex != '{': erra('Error de Sintaxis', 'se esperaba abrir \"{\"')
    tok, lex = scanner()
    if lex != '}': estatutos()
    if lex != '}':erra('Error de Sintaxis', 'se esperaba cerrar \"}\"')
...
def funcs():
        ...
        tok, lex = scanner()      
        if lex != '(': erra('Error de Sintaxis', 'se esperaba parentisis abierto \"(\"') 
        tok, lex = scanner()      
        if lex != ')': params()
